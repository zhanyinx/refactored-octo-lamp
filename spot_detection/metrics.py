...
def weighted_f1_coordinates(pred: np.ndarray, true: np.ndarray, cell_size: int, weight: float = 1) -> float:
...
def compute_score(true: np.ndarray, pred: np.ndarray, cell_size: int, weight: float) -> pd.DataFrame:
    ...
    for p, t in zip(true, pred):
        ...
        weighted_f1_coordinates_.append(weighted_f1_coordinates(p, t, cell_size, weight))
