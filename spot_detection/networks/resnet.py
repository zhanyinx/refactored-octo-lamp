...
def resnet(n_channels: int = 3) -> tf.keras.models.Model:
    ...
    # Down: 512 -> 256
    x = conv_block(inputs=inputs,filters=2 ** (i), n_convs=2, dropout=dropout)
    x = residual_block(inputs=x,filters= 2** (i))
    x = conv_block(inputs=x,filters=2 ** (i+1), n_convs=2, dropout=dropout)
    ...
    # Connected
    x = conv_block(inputs=x,filters=2 ** (i+2), n_convs=2, dropout=dropout)
